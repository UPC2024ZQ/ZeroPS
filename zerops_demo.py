...
parser.add_argument('--object_name', type=str, default="Chair", help="Object name (default: 'Chair')")
parser.add_argument('--part_names', type=str, nargs='+', default=["arm", "back", "leg", "seat", "wheel"], help="List of part names")
parser.add_argument('--T_merge', type=float, default=0.3, help="Threshold for merging")
parser.add_argument('--with_knn', action='store_true', help="【Better visualization】Whether to use KNN")
...
sam = sam_model_registry["vit_h"](checkpoint="/home/yuhengxue/zerops/models/sam_vit_h_4b8939.pth").to(
    device=device
)
...
cfg.merge_from_list(["MODEL.WEIGHT", "/home/yuhengxue/zerops/models/glip_large_model.pth"])
...
def bfs_view_sequence(adjacency_list, start_node): # 基于BFS生成视角拓展序列，确保相邻视角有共视区域
    queue = [start_node]
    visited = set()
    extend_sequence = []
    while queue:
...
def visualize_groups_3d(xyz, components, save_dir, name): #3D点云可视化（给不同零件分配颜色）
...
def save_colored_pc(file_name, xyz, rgb): #保存点云
...
def normalize_pc_o3d(pc_file): #点云归一化
...
def get_largest_connected_domain(mask): #2D连通域筛选（保留2D掩码中最大的连通部分，过滤噪声）
...
def get_lcd(xyz, n=1): #3D最大连通域（用DBSCAN聚类，筛选有效3D零件，去除零散噪点）
...
def compute_centroid(point_cloud): #计算点云质心（用于生成SAM的提示点，提升分割稳定性）
...
def point_feature_mapping_view(pc_idx, feature): #将3D点云的特征映射到2D图像像素
...
def generate_groups_3d_starting( #从起始视角生成初始3D组
    starting_view_name,
    view_name,
    sam_predictor,
    xyz,
...
def single_view_extension( # 从单视角拓展3D组
...
def labeling( #给无标签3D零件分配语义标签和实例标签
    model_dir,
    xyz,
    rgb,
    pc_idx,
...
def render_pc(xyz, rgb, device, resolution=800): #多视角渲染
    pc = Pointclouds(
...
def merging(groups_3d, T_merge, xyz): #合并所有视角生成的3D组，得到无标签3D零件
...
def predict(input_pc_file, category, part_names, model_dir="tmp"): #零样本3D零件分割的完整流程
    ...
    # The KNN is used to assign labels to the unclassified points, 
    # aiming to enhance the visual appeal of the results. 
    # Note that this operation is not included in the segmentation evaluation (False).
    if with_knn:
        unlabel_seg = knn_propagation_background(unlabel_seg, xyz)
    ...
    for i in np.unique(unlabel_seg):
        if i == -1:
            continue
        parts.append(np.where(unlabel_seg == i)[0])
    visualize_groups_3d(xyz, parts, model_dir, f"unlabel_seg.ply")
    ...
    if with_labeling:
        ...
        # vis ins & sem seg
        os.makedirs(f'{model_dir}/sem_seg', exist_ok=True)
        os.makedirs(f'{model_dir}/ins_seg', exist_ok=True)
        for sem_id, p_name in enumerate(part_names):
            sem_idx = np.where(sem_seg == sem_id)[0]
            visualize_groups_3d(xyz,[sem_idx], f'{model_dir}/sem_seg', f"{p_name}.ply")
            ins_idxs = []
            for k in np.unique(ins_seg[sem_idx]):
                if k == -1:
                    continue
                ins_idx = np.where(ins_seg == k)[0]
                ins_idxs.append(ins_idx)
            visualize_groups_3d(xyz, ins_idxs, f'{model_dir}/ins_seg', f"{p_name}.ply")

if __name__ == "__main__":
    
    root = './demo_data'
    predict(f"{root}/{object_name}/pc.ply",object_name,part_names,model_dir=f'{root}/{object_name}')
